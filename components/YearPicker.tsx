  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      var closestYearDiv = container?.childNodes[0].childNodes.item(
        2026 - Number(closestYear.textContent)
      ) as HTMLElement;
    ...
    <div className="tw-flex tw-flex-col tw-justify-center tw-items-center tw-gap-y-20">
      <div className="tw-w-[300px] tw-h-[340px] tw-text-white tw-overflow-scroll tw-text-center tw-relative container tw-py-[60px] tw-select-none">
        <ul className="scroller">
          {[...Array(127)].map((_, index) => {
            return (
              <li
                key={index}
                className={`tw-my-5 text-thin tw-text-[35px] select-years tw-opacity-50 ${index === 0 || index === 1 ? "!tw-opacity-0" : ""}`}
              >
                {2026 - index}
              </li>
            );
      ...
      <a className="tw-group tw-relative tw-inline-block focus:tw-outline-none focus:tw-ring tw-cursor-pointer">
        <span className="tw-absolute tw-inset-0 tw-translate-x-1.5 tw-translate-y-1.5 tw-bg-yellow-300 tw-transition-transform group-hover:tw-translate-x-0 group-hover:tw-translate-y-0"></span>

        <span
          className="tw-relative tw-inline-block tw-border-2 tw-border-current tw-px-8 tw-py-3 tw-text-sm tw-font-bold tw-uppercase tw-tracking-widest tw-text-black group-active:tw-text-opacity-75"
